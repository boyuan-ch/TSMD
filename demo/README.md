...
frame-level video-summarization predictions for four robustness examples. Each
video compares TripleSumm (Baseline) and TSMD-Mix (Ours) under clean and
...
- Select any of the four robustness examples.
...
`data/manifest.json` defines the video order, YouTube IDs, and paths to the four
self-contained per-video JSON files. Robustness curves are
