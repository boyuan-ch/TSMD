...
def evaluate_summary(pred_score, gt_score, mask, return_per_video=False):
    ...
    if return_per_video:
        return np.mean(ktau_list), np.mean(srho_list), ktau_list, srho_list
...
def evaluate_highlight(pred_score_list, gt_score_list, mask, fps=1, return_per_video=False):
    ...
    if return_per_video:
        return map50, map15, ap50_list, ap15_list
    return map50, map15